...
def load_model_for_infer(model_class, arg_path, arg_file_name):
    pt_path = os.path.join(arg_path, arg_file_name)
...
if __name__ == "__main__":
    path = "checkpoint/0501"
    file_name = "18983_time_1777921873.pt"

    tokenizer = GPT2TokenizerFast.from_pretrained("gpt2",local_files_only=True)

    device = 'cuda' if torch.cuda.is_available() else 'cpu'

    model = load_model_for_infer(GPT2Model, path, file_name)
    model.eval()
    # claude prompt
    prompts = [
